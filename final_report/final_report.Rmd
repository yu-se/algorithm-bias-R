...
target_race_me <- df %>% filter(!(hatespeech==1) & target_race == "native_american")
target_race_me <- within(target_race_me, hatespeech[hatespeech == 2] <- 1)

set.seed(1122)

df_model <- target_race_me
df_model <- subset(target_race_me, select = -c(target_race, comment_id, annotator_id ))
...
model <- glm(hatespeech ~ annotator_race+target_gender+annotator_age, data = train)
