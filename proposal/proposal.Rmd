...
```
```{r}
anno2 <- subset(df_wo_red, select = c(target_race, annotator_race))
anno2 <- drop_na(anno2, target_race)
anno2
```
...
```{r}
anno_wo_red <- subset(df_wo_red, select = -c(annotator_race_asian, annotator_race_black, annotator_race_latinx, annotator_race_middle_eastern, annotator_race_native_american, annotator_race_pacific_islander, annotator_race_white, annotator_race_other))
...
target_disability_unspecific, target_disability_other, target_disability, text, platform,
sentiment, respect, insult,
humiliate, status, dehumanize,
violence, genocide, attack_defend,
hatespeech, hate_speech_score, text,
infitms, outfitms, annotator_severity,
std_err, annotator_infitms, annotator_outfitms,
hypothesis, comment_id, annotator_income_.10k, annotator_income_10k.50k, annotator_income_50k.100k,
...
annotator_ideology_no_opinion, annotator_education_some_high_school, annotator_education_high_school_grad,
...
anno_wo_red <- subset(df_wo_red, select = -c(annotator_religion_atheist, annotator_religion_buddhist, annotator_religion_christian, annotator_religion_hindu, annotator_religion_jewish, annotator_religion_mormon, annotator_religion_muslim, annotator_religion_nothing, annotator_religion_other))
anno_wo_red
```
...
income <- anno[,combo]
income['income_level'] <- rep(0, 49875)
...
# religion
```{r}
relig <- c('target_religion_atheist',
...
traget_relig <- comm[,relig]
traget_relig['relig'] <- rep(0, length(comm$comment_id))
                       
traget_relig <- within(traget_relig, relig[target_religion_atheist == 'True']<-'atheist')
traget_relig <- within(traget_relig, relig[target_religion_buddhist == 'True']<-'uddhist')
traget_relig <- within(traget_relig, relig[target_religion_hindu == 'True']<-'hindu')
traget_relig <- within(traget_relig, relig[target_religion_jewish == 'True']<-'jewish')
traget_relig <- within(traget_relig, relig[target_religion_mormon == 'True']<-'mormon')
traget_relig <- within(traget_relig, relig[target_religion_muslim == 'True']<-'muslim')
traget_relig <- within(traget_relig, relig[target_religion_other == 'True']<-'other')

traget_relig <- traget_relig %>% filter(relig != 0 )
head(traget_relig)
```
```{r}
ggplot(traget_relig, aes(x=relig))+
  geom_bar(fill='dark blue') +
  labs(
    x = "Target Religion",
    y = "Counter",
...
both <- within(both, anno_race[annotator_race_middle_eastern == 'True']<-'middle_eastern')
both <- within(both, anno_race[annotator_race_pacific_islander == 'True']<-'pacific_islander')
...
```{r}
ggplot(race_count, aes(x=seq(0, 1, by = 0.1), y=race, fill=p))+
