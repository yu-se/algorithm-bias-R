...
In this project, we are using data published in an article made in Berkeley as explained in section 1 link to both article and dataset are attached at the end of this section.
Here a glimpse of the data
...
The data contain 131 variables and 135,556 rows. To be able to work with it, we filter it to 30 relevant variables, that hold information both on the annotator and on the post itself, a lot of time will be references as “target”.  In this data, you can find many types of variables for example the “hate_speech_score” variable is a numerical variable, while the “annotator_race” variable is a categorical one.
...
As we can in the graph below there is a big difference between annotators and targets distributions.
Clearly, most of the annotators are whites while most of the targets are blacks.
...
As we can see in the age of the annotators approximately normal distribution is around mu=30.
...
In the next plot, it looks like the way that all the races measure hate speech is the same but is it so? 
...
We will try to predict the significance of the difference between the way annotators from different backgrounds are choosing to score hate speech. To do so we will try to find which features give us the most variance explained.

For the prediction above we will use basic statistical methods such as Pearson correlation, the covariance between variables, and linear regression models.

The results we need to see to support our hypothesis are strong connections between the way people see hate speech and their background.
We choose to divide the work as follows: one of us is more responsible for statistical concepts while the other is more focused on the visualization and explanation parts.
